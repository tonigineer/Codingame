...
codingame.com). All of it is generic: every CLI takes `--bot <crate>`
(default `trollfarm`). **Convention:** bot crates are named after the
puzzle's URL slug (`codingame.com/multiplayer/bot-programming/<slug>`), so
the crate name *is* the puzzle id; `ide/_browser.PUZZLE_IDS` holds the
exceptions (e.g. `trollfarm` → `spring-challenge-2026-troll-farm`), and
`--puzzle` overrides both.
...
dir: `bots/<bot>/eval/` and `bots/<bot>/replays/`.
...
```bash
uv run --project tools tools/local/validate.py 100
...
Crates are resolved as `bots/<pkg>` relative to the repo root.
...
- **`validate.py`** — N reproducible games (parallel) against each opponent
  ...
  Everything lives in `bots/<bot>/codingame/`: the referee jar
  ...
  on gives you held-out validation.
    ...
    ```bash
    uv run --project tools tools/local/validate.py                  # default roster
...
Generic across games: each CLI takes `--bot <crate>` (default `trollfarm`);
the puzzle id is the crate name itself unless `_browser.PUZZLE_IDS` or
`--puzzle` says otherwise. The browser tools
...
- **`_browser.py`** — the shared **library** (no CLI): path anchors, the
  puzzle-id convention, and the selenium primitives — driver attach/launch,
...
- **`play.py`** — play ONE game: injects `bots/<bot>/src/main.rs.flattened`
...
- **`bench.py`** — **THE standard bench**: replays the pinned arena seeds
  (`--seeds-file`, default `bots/<bot>/eval/arena_bench_seeds.json`: 5 lost +
...
- **`fetch_replay.py`** — one replay → summary (players, scores, referee
  seed, frames) + full JSON to `bots/<bot>/replays/replay_<id>.json`. Two
...
- **`fetch_player_games.py`** — download all (or `--limit N`) of a player's
  arena replays to `bots/<bot>/replays/<player>/`; `--list-top N` lists the
  leaderboard. Also `just get-games <player>`.

    ```bash
    uv run --project tools tools/ide/fetch_player_games.py tonigineer
    ```
